...
class DRQNPolicy(BasePolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, scale=False,
        ...
        super(DRQNPolicy, self).__init__(sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=reuse, scale=scale,
                                        obs_phs=obs_phs)
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, layers=None,
        ...
        with tf.variable_scope("model", reuse=reuse):
            with tf.variable_scope("action_value"):
                ...
                # Do I just attach the RNN right here?
                # This is after the first 2 FC layers, and before the last final FC layer
                # How many observations to send in
                #time_steps = 4
                # hidden LSTM units
                #num_units = 128
                # input size
                #n_input = len(action_out)
                #n_classes = layers[-1]
