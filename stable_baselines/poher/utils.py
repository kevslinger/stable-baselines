...
# KEVIN UPDATE
# I WANT OBS DESIRED ACHIEVED
KEY_ORDER = ['observation', 'desired_goal', 'achieved_goal']

class POHERGoalEnvWrapper(object):
    """
    ...
    def __init__(self, env):
        super(POHERGoalEnvWrapper, self).__init__()
        self.env = env
        ...
        if isinstance(self.spaces[0], spaces.MultiBinary):
            ...
            self.her_observation_space = spaces.MultiBinary(total_dim)
            # for dqn, we don't want it to see the achieved goal because that's a state!
            dqn_total_dim = self.obs_dim + self.goal_dim
            self.dqn_observation_space = spaces.MultiBinary(dqn_total_dim)
    ...
    def convert_dict_to_obs(self, obs_dict):
        ...
        if isinstance(self.her_observation_space, spaces.MultiDiscrete):
    ...
    def convert_obs_to_dict(self, observations):
        ...
        #])
        # KEVIN UPDATE
        # CHANGING TO OBSERVATION THEN DESIRED THEN ACHIEVED
        return OrderedDict([
            ('observation', observations[:self.obs_dim]),
            ('desired_goal', observations[self.obs_dim:self.obs_dim + self.goal_dim]),
            ('achieved_goal', observations[self.obs_dim + self.goal_dim:]),
        ])
