...
from .utils import POHERGoalEnvWrapper


class POHER(BaseRLModel):
    """
    ...
    def __init__(self, policy, env, model_class, n_sampled_goal=4,
        ...
        assert not isinstance(env, VecEnvWrapper), "POHER does not support VecEnvWrapper"
        ...
        assert issubclass(model_class, OffPolicyRLModel), \
            "Error: POHER only works with Off policy model (such as DDPG, SAC, TD3 and DQN)."
    ...
    def _create_replay_wrapper(self, env):
        ...
        """
        if not isinstance(env, POHERGoalEnvWrapper):
            env = POHERGoalEnvWrapper(env)
    ...
    def learn(self, total_timesteps, callback=None, log_interval=100, tb_log_name="POHER",
